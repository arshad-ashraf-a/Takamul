	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			jsonReader: {
			    ...
			    page: function (obj) { return jQuery(grid_selector).jqGrid('getGridParam', 'page'); },
			    total: function (obj) { obj[0].TotalCount; if (obj.length == 0) return 0; else return Math.ceil(obj[0].TotalCount / jQuery(grid_selector).jqGrid('getGridParam', 'rowNum')); },
			...
			ExpandColumn: null,
			tree_root_level : 0,
			prmNames: { page: "nPage", rows: "nRows", sort: "sColumnName", order: "sColumnOrder", search: "_search", nd: "nd", id: "id", oper: "oper", editoper: "edit", addoper: "add", deloper: "del", subgridid: "id", npage: null, totalrows: "totalrows" },
			forceFit : false,
			gridstate : "visible",
			cellEdit: false,
			cellsubmit: "remote",
		...
		var eg = $("<div class='ui-jqgrid ui-widget ui-widget-content ui-corner-all top-buffer bottom-buffer'></div>");
				...
				case "script":
					$.ajax($.extend({
						...
						error: function (xhr, st, err) {
						    if (xhr.status == 401)
						    {
                                alert("You are not authorized!")//TODO redirect to unauthorized page
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.pgbuttons===true) {
				...
				pgl += "<td id='" + po[0] + "' class='ui-pg-button ui-corner-all'><span class='ui-icon fa fa-angle-left'></span></td>";
				pgl += "<td id='" + po[1] + "' class='ui-pg-button ui-corner-all'><span class='ui-icon fa fa-angle-double-left'></span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='" + po[2] + "' class='ui-pg-button ui-corner-all'><span class='ui-icon fa fa-angle-right'></span></td>";
				pgl += "<td id='" + po[3] + "' class='ui-pg-button ui-corner-all'><span class='ui-icon fa fa-angle-double-right'></span></td>";
